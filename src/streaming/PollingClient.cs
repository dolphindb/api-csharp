...
namespace dolphindb.streaming
{
    public class PollingClient : AbstractClient
    {
        ...
        override protected bool doReconnect(SubscribeInfo subscribeInfo, Site site)
        {
            try
            {
                BlockingCollection<List<IMessage>> queue = subscribeInternal(site.host, site.port, subscribeInfo.getTableName(), subscribeInfo.getActionName(), null, subscribeInfo.getMsgId() + 1, true, subscribeInfo.getFilter(), subscribeInfo.getDeseriaLizer(), subscribeInfo.getUser(), subscribeInfo.getPassword(), false, subscribeInfo.getMsgAsTable());
        ...
        public TopicPoller subscribe(string host, int port, string tableName, string actionName, long offset, bool reconnect, IVector filter, StreamDeserializer deserializer = null, string user = "", string password = "", bool msgAsTable = false)
        {
            BlockingCollection<List<IMessage>> queue = subscribeInternal(host, port, tableName, actionName, null, offset, reconnect, filter, deserializer, user, password, true, msgAsTable);
