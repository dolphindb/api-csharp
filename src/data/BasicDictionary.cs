    ...
    public class BasicDictionary : AbstractEntity, IDictionary
    {
        ...
        public DataTable toDataTable()
        {
            ...
            for (int i = 0; i < itemCount; i++)
            {
                ...
                if(keyType == DATA_TYPE.DT_ANY)
                {
                    dr["dict_key"] = keys[i].getString();
                }
                else
                {
                    dr["dict_key"] = Utils.getRowDataTableObject(keys[i]);
                }
                if (keyType == DATA_TYPE.DT_ANY)
                {
                    dr["dict_value"] = values[i].getString();
                }
                else
                {
                    dr["dict_value"] = Utils.getRowDataTableObject(values[i]);
                }
                dt.Rows.Add(dr);
            }
            return dt;
