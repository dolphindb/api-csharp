    ...
    public class BasicTable : AbstractEntity, ITable
    {
        ...
        public BasicTable(IList<string> colNames, IList<IVector> cols)
        {
            if (colNames.Count != cols.Count)
                throw new Exception("the size of colNames must be equal than cols");
            setColName(colNames);
            setColumns(cols);
        }
        ...
        public DataTable toDataTable()
        {
            ...
            for (int rowIndex = 0; rowIndex < rows(); rowIndex++)
            {
                ...
                {
                    IVector vector = columns_[colIndex];
                    dr[colIndex] = Utils.getRowDataTableObject(vector, rowIndex);
                }
                dt.Rows.Add(dr);
            }
            return dt;
