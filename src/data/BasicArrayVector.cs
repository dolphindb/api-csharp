...
namespace dolphindb.data
{
    public class BasicArrayVector : AbstractVector
    {
        ...
        public BasicArrayVector(DATA_TYPE type, int extra = -1) : base(DATA_FORM.DF_VECTOR)
        {
            this.type = type;
        ...
        public void deserialize(int rows, ExtendedDataInput @in)
        {
             ...
             while(rowsRead < rows)
            {
                ...
                while(offect < totalBytes)
                {
                    ...
                    {
                        for (int i = 0; i < curRows; ++i) {
                            int curRowCells = BitConverter.ToInt16(buffer, i * 2);
                            rowIndices[rowsRead + rowsReadInBlock + i] = prevIndex + curRowCells;
                            prevIndex += curRowCells;
                        }
                    }
                    else
                    {
                        for (int i = 0; i < curRows; ++i)
                        {
                            int curRowCells = BitConverter.ToInt32(buffer, i * 4);
                            rowIndices[rowsRead + rowsReadInBlock + i] = prevIndex + curRowCells;
                            prevIndex += curRowCells;
                        }
                    }
        ...
        public BasicArrayVector(int[] index, IVector value):base(DATA_FORM.DF_VECTOR)
        {
            ...
            if (Utils.getCategory(valueType) == DATA_CATEGORY.DENARY)
                this.scale_ = this.valueVec.getScale();
        ...
        public BasicArrayVector(List<IVector> value, int extra = -1):base(DATA_FORM.DF_VECTOR)
        {
            this.scale_ = extra;
            ...
            this.valueVec = (AbstractVector)BasicEntityFactory.instance().createVectorWithDefaultValue(valueType, count, extra);
            int index = 0;
            int curRows = 0;
        ...
        {
            return valueVec.getExtraParamForType();
        }

        public override IVector getSubVector(int[] indices)
        {
            BasicArrayVector arrayVector = new BasicArrayVector(type, scale_);
            foreach(int  i in indices)
            {
                arrayVector.append(getSubVector(i));
            }
            return arrayVector;
        }
    }
