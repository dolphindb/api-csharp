...
namespace dolphindb.data
{
    ...
    public class BasicMinuteVector : BasicIntVector
    {
        ...
        public override void set(int index, string value)
        {
            ...
            int v;
            if (int.TryParse(value, out v) && v >= 0 && v <= 1440)
            {
                values[index] = v;
                return;
            }
            setNull(index);
            Console.WriteLine("[Warning]:Invalid value " + value + " for set(int index, string value) function and the value is set to null. Valid value must be [mm] or [HH:mm] format.");
        }
