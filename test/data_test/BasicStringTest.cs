...
namespace dolphindb_csharp_api_test.data_test
{
    ...
    public class BasicStringTest
    {
        ...
        {
            BasicString str = (BasicString)conn.run("mt=table(take(1..1000000,1000000) as id,take(`fyf``ftf`ddtjtydtyty`通常都是,1000000) as str,take(`fyf``ftf`ddtjtydtyty`通常都是,1000000) as str1," +
        ...
        {
            conn.run("a=table(100:0, `id`value`memo, [INT, DOUBLE, BLOB])");
            conn.run("insert into a values(10,0.5, '[{\"name\":\"shily\",\"sex\":\"女\",\"age\":\"23\"},{\"name\":\"shily\",\"sex\":\"女\",\"age\":\"23\"},{\"name\":\"shily\",\"sex\":\"女\",\"age\":\"23\"}]')");
            BasicTable tb = (BasicTable)conn.run("a");
        ...
        [TestMethod]
        public void blob_imemory_table_upload_download()
        {
            conn.run("st = table(100:0, `id`memo, [INT,  BLOB])");
            ...
            conn.run("tableInsert{st}", arg);

            BasicTable st = (BasicTable)conn.run("st");
            Console.WriteLine(((BasicTable)st).getString());
        }
  
    [TestMethod]
    public void blob_imemory_table_upload_download_bigdata()
    {
        conn.run("st = table(blob(`d`f) as blob)");
        ...
        BasicString str = (BasicString)conn.run("mt=table(take(1..1000000,1000) as id,take(`fyf``ftf`ddtjtydtyty`通常都是,1000) as str,take(`fyf``ftf`ddtjtydtyty`通常都是,1000) as str1," +
