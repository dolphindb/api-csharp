...
namespace dolphindb_csharp_api_test.data_test
{
    ...
    public class BasicMatrixTest
    {
        ...
        [TestMethod]
        public void test_BasicComplexMatrix()
        {
            BasicComplexMatrix bcv = new BasicComplexMatrix(0,0);
        ...
        [TestMethod]
        public void test_BasicComplexMatrix_list()
        {
            List<Double2[]> list = new List<Double2[]>();
            list.Add(new Double2[] { new Double2(1.0, 9.2), new Double2(-3.8, -7.4), new Double2(0, 0), new Double2(double.MinValue, double.MinValue) });
            BasicComplexMatrix bcv = new BasicComplexMatrix(2,2,list);
            Assert.AreEqual(2, bcv.rows());
            Assert.AreEqual("-3.8-7.4i", bcv.get(1,0).getString());
            Assert.AreEqual(new BasicComplex(0, 0), bcv.get(0,1));
            Assert.AreEqual(new BasicComplex(double.MinValue, double.MinValue), bcv.get(1,1));
            String re = null;
            try
            {
                BasicComplexMatrix bcv1 = new BasicComplexMatrix(3, 2, list);
            ...
            Assert.AreEqual(true, re.Contains("the total size of list must be equal to rows * columns"));

        }

        [TestMethod]
        public void test_BasicComplexMatrix_set()
        {
            List<Double2[]> list = new List<Double2[]>();
            list.Add(new Double2[] { new Double2(1.0, 9.2), new Double2(-3.8, -7.4), new Double2(0, 0), new Double2(double.MinValue, double.MinValue) });
            BasicComplexMatrix bcv = new BasicComplexMatrix(2, 2, list);
            Assert.AreEqual(new BasicComplex(1.0, 9.2), bcv.get(0,0));
            bcv.set(0,0, new BasicComplex(0, 0));
            Assert.AreEqual(new BasicComplex(0, 0), bcv.get(0,0));
            String re = null;
            try
            {
                bcv.set(0,0, new BasicBoolean(true));
        ...
        [TestMethod]
        public void test_BasicComplexMatrix_setNull()
        {
            List<Double2[]> list = new List<Double2[]>();
            list.Add(new Double2[] { new Double2(1.0, 9.2), new Double2(-3.8, -7.4), new Double2(0, 0), new Double2(double.MinValue, double.MinValue) });
            BasicComplexMatrix bcv = new BasicComplexMatrix(2, 2, list);
            Assert.IsFalse(bcv.isNull(0,1));
            bcv.setNull(0, 1);
            Assert.IsTrue(bcv.isNull(0, 1));
            Console.WriteLine(bcv.getString());
            Assert.AreEqual("#0        #1\n1+9.2i      \n-3.8-7.4i   \n", bcv.getString());
        }
    }
}
