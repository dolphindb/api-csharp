...
namespace dolphindb_csharp_api_test.streamReverse_test
{
    ...
    [TestInitialize]
        ...
        public void PrepareStreamTable_StreamDeserializer(String dataType)
        {
        ...
        public static void checkResult1(DBConnection conn)
        {
            ...
            Assert.AreEqual(except.rows(), res.rows());
            Assert.AreEqual(except1.rows(), res1.rows());
            for (int i = 0; i < except.columns(); i++)
            {
                Assert.AreEqual(except.getColumn(i).getString(), res.getColumn(i).getString());
                Assert.AreEqual(except1.getColumn(i).getString(), res1.getColumn(i).getString());
            }
        }
      ...
      //subscribe(string host, int port, string tableName, string actionName, MessageHandler handler, long offset, bool reconnect, IVector filter, StreamDeserializer deserializer = null, string user = "", string password = "")
        ...
        public class Handler_StreamDeserializer_array : MessageHandler
        {
            private StreamDeserializer deserializer_;
            ...
            public void doEvent(IMessage msg)
            {
                try
                {
                    ...
                    String dataType = message.getEntity(1).getString().Replace(",,", ",NULL,").Replace("\\[,", "[NULL,").Replace(",]", ",NULL]").Replace(',', ' ');
                    String script = null;
        ...
        [TestMethod]
        public void Test_ThreadPooledClient_subscribe_StreamDeserializer_streamTable_arrayVector_BOOL()
        {
